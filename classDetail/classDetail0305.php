<?
	include "/home/edufim/www/module/login/head.php";
?>

<div class="classDetail_wrap">
	<div class="dp_f dp_cc c_w bora classDetail_tit">통증/재활</div>

	<div class="sec">
		<p class="c_bora01 bold2 txt-c classDetail_sub_tit">추천대상</p>
		<div class="verti_line bora01"><!-- 선--></div>
		<ul class="classDetail_box_wrap box_03 dp_sb dp_c">
			<li class="dp_f dp_c dp_cc">① 근피로를 줄이고 싶다.</li>
			<li class="dp_f dp_c dp_cc">② 운동 도중 통증이 발생한다.</li>
			<li class="dp_f dp_c dp_cc">③ 운동 자체를 수행할 수 없다.</li>
		</ul>
		<div class="under_arrow dp_f dp_c dp_cc">
			<img src="/images/classDetail_arr.png" alt="">
		</div>
		<ul class="classDetail_box_wrap box_01 dp_sb dp_c">
			<li class="c_bora01 bold2 dp_f dp_c dp_cc">운동접근법 이외에 수기로 통증 및 재활을 만들어 내는 방법</li>
		</ul>
	</div>

	<div class="sec">
		<p class="c_bora01 bold2 txt-c classDetail_sub_tit">맞춤강좌</p>
		<div class="verti_line bora01"><!-- 선--></div>
		<ul class="classDetail_box_wrap box_02 dp_sb dp_c">
			<li class="dp_f dp_c dp_cc">PTI 연부조직 Part</li>
			<li class="dp_f dp_c dp_cc">관절 Part</li>
		</ul>
	</div>

</div>

<style>
	.classDetail_wrap .classDetail_tit {max-width: 162px;}
</style>
