...
$query = "SELECT l.class_uid, l.progress, c.title 
    FROM ks_learning l
    ...
    <div class="s_center dp_sb">
        ...
        <div class="s_cont">
            <div class="s_cont_tit">
                <span class="m_s_cont_tit">수료증 발급</span>
                <ul class="s_cont_tabbtn dp_f">
                    ...
                    <li class="on"><a href="/mypage/certCompletion/">수료증 발급</a></li>
                    <li><a href="/mypage/certLicense/">자격증 발급</a></li>
                </ul>
            </div>
            <p class="c_gry04 f14 m_40">진도율 80% 이상시 시험 응시 가능하며, 시험 합격시 수료증을 발급 받을 수 있습니다.</p>
            <div class="tableWrap">
                <a href="javascript:void(0)" class="m_10 f14 testStatus dp_f dp_c dp_cc on bora c_w" onclick="searchCert()">수료증 조회</a>
                    ...
                    <tbody>
                        ...
                        <? if ($row_num > 0) { ?>
                            ...
                            foreach ($row_arr as $key => $row) {
                                ...
                                $query2 = "SELECT * FROM ks_cert_completion WHERE userid='$GBL_USERID' AND class_uid='$class_uid'";
                                $row_num2 = sqlRowCount($query2);
                                $row_num2 = 1;

                                $testTxt = ($row_num2 > 0) ? "수료증 발급" : "시험 응시";
                            ?>
                                <tr>
                                    <td><?= $row['title'] ?></td>
                                    <td><?= $row['progress'] ?>%</td>
                                    <td>
                                        <div class="testStatus dp_f dp_c dp_cc <? if ($row_num2 > 0) echo 'on certifi_printBtn' ?>" data-tid="<?= $row['class_uid'] ?>"><?= $testTxt ?></div>
                                    </td>
                                </tr>
                            <? } ?>
                        <? } else { ?>
...
<script>
    ...
    function tema(t) {
        $('#classConsFrame').html("<iframe src='../cert/completion.php?uid=" + t + "' name='' style='width:100%;height:730px;' frameborder='0' scrolling='auto'></iframe>");
