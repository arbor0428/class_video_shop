...
// $query = "SELECT cert.*, class.title ctitle FROM ks_cert_completion  $query_ment $sort_ment";
$query = "SELECT cert.*, c.title ctitle FROM ks_cert_completion cert JOIN ks_class c ON cert.class_uid=c.uid order by cert.rDate desc";
			...
			<table cellpadding='0' cellspacing='0' border='0' width='100%' class='listTable' style='min-width:900px;margin:5px 0;'>
				...
				if ($total_record) {
					...
					while ($row = mysql_fetch_assoc($result)) {
						if ($row['status'] == 0) $statusTxt = "<span class='ico10'>수료전</span>";
						if ($row['status'] == 1) $statusTxt = "<span class='ico03'>시험통과</span>";
						if ($row['status'] == 2) $statusTxt = "<span class='ico07'>발급완료</span>";
				?>
						<tr class='grayLine'>
							<td><?= $i ?></td>
							<td><?= $statusTxt ?></td>
