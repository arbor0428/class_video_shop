...
<form name='frm01' class="user wid100" method='post' action='./proc.php' ENCTYPE="multipart/form-data">
    ...
    <div class="tbl-st">
        ...
        <div class="cols">
            ...
            <div class="cols_90 cols_">
                <div class="form-group">
                    <select name="kollus_video_id" id="kollus_video_id" class="form-control input-50">
                        <option value="-1">없음</option>
                        <?
                        $query = "SELECT title, kollus_video_id FROM ks_class WHERE tuid='$GBL_UID'";
                        $arr_classes = sqlArray($query);
                        foreach ($arr_classes as $class) {
                        ?>
                            <option value="<?= $class['kollus_video_id'] ?>" <? if ($class['kollus_video_id'] == $kollus_video_id) echo "selected"; ?>><?= $class['title'] ?></option>
                        <?
                        }
                        ?>
                    </select>
