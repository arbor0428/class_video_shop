...
<form name='frm01' class="user" method='post' ENCTYPE="multipart/form-data">
    ...
    <div class="tbl-st">
        ...
        <div class="cols">
            <div class="cols_10 cols_ th"><span class='eqc'>*</span>클래스 선택</div>
            <div class="cols_90 cols_">
                <div id="class_info">
                    <a href="javascript:void(0)" onclick="formModal()">
                        <span class="ico05">검색 <i class="fas fa-search"></i></span>
                    </a>
                </div>
            </div>
        </div>
        ...
        <div class="cols">
            ...
            <div class="cols_30 cols_">
                <div class="form-group">
                    합: <input type="text" name="price_tmp" class="txt-r" value="0" readonly disabled>
            ...
            <div class="cols_30 cols_">
                <div class="form-group">
                    합: <input type="text" name="discountPrice_tmp" class="txt-r" value="0" readonly disabled>
...
<script>
    ...
    const formModal = function(value1) {
        ...
        if(document.frm01['class_uids[]'] === undefined) {
            class_uids = ''

        } else if (document.frm01['class_uids[]'].length === undefined) {
            class_uids = document.frm01['class_uids[]'].value

        } else {
            document.frm01['class_uids[]'].forEach(ele => {
                class_uids += ele.value + ',';
            });
            class_uids = class_uids.substring(0, class_uids.length - 1)
        }

        console.log(class_uids);

        $("#multiBox").css({
            "width": "90%",
            "max-width": "1200px"
        });
        $('#multi_ttl').text('ALL 클래스 목록');
        $('#multiFrame').html("<iframe src='./write2.php?class_uids=" + class_uids + "' name='write2' style='width:100%;height:680px;' frameborder='0' scrolling='auto'></iframe>");
        $('.multiBox_open').click();
    }

    const removeClass = function(uid) {
        parent.document.frm01.price_tmp.value = comma(parseInt(uncomma(parent.document.frm01.price_tmp.value)) - parseInt(uncomma($('#class_info_' + uid).data().price)))
        parent.document.frm01.discountPrice_tmp.value = comma(parseInt(uncomma(parent.document.frm01.discountPrice_tmp.value)) - parseInt(uncomma($('#class_info_' + uid).data().dprice)))
        $('div').remove('#class_info_' + uid);
    }
