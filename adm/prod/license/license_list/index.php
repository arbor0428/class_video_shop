...
$query = "SELECT c.*, c1.title c1t, c2.title c2t, c3.title c3t, l.*
FROM ks_class c
LEFT JOIN ks_class_cade01 c1 ON c.cade01=c1.uid
LEFT JOIN ks_class_cade02 c2 ON c.cade02=c2.uid
LEFT JOIN ks_class_cade03 c3 ON c.cade03=c3.uid
...
$result = mysql_query("$query WHERE c.prod_type='CLASS_ONLINE' AND l.class_uid IS NOT NULL ORDER BY is_required DESC, sort") or die(mysql_error());
...
<div id="content-wrapper" class="d-flex flex-column">
                                        ...
                                        <table width="100%" cellpadding='0' cellspacing='0' border='0' width='100%' class='listTable'>
                                            ...
                                            ?>
                                                <tr class='grayLine'>
                                                    <td>
                                                        <label class="switch">
                                                            <input type="checkbox" name="status_<?= $uid ?>" value='<?= $row['uid'] ?>' class="switch-input" style="position:fixed" <? if ($row['is_required'] == 1) echo 'checked'; ?>>
                                                            <span class="switch-label" data-on="필수" data-off="선택"></span>
    ...
    <script>
        ...
        const reg_list = function() {
            location.href = '/adm/prod/license/'
        }
