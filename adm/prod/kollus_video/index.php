...
$cond = "";
if (isset($f_cat) && $f_cat != '') $cond .= "WHERE category_name='$f_cat'";

$query = "SELECT * FROM kollus_video $cond ORDER BY status=2 DESC, reg_date DESC";
...
<script>
    ...
    const select = function(id) {
        if (parent.document.getElementById(value1) === null) return;
    ...
    const searchChk = function () {
        const form = document.kollus_frm;
        parent.document.frm01.f_cat.value = form.f_cat.value;
        form.target = '';
