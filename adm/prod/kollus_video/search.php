...
<div class='searchBox' style='width:100%;'>
    <div style="float:left;">
        ...
        카테고리 
        <select name="f_cat" id="f_cat" class="form-control" onchange="searchChk()" style="width:300px;">
            <option value=''>전체</option>
            <?
            $query = "SELECT category_name FROM kollus_video GROUP BY category_name";
            $cat_arr = sqlArray($query);

            foreach ($cat_arr as $key => $cat) {
                $cat_name = $cat['category_name'];
            ?>
            <option value='<?= $cat_name ?>' <? if ($f_cat == $cat_name) echo 'selected'; ?>><?= $cat_name ?></option>
            <?
            }
            ?>
        </select>
