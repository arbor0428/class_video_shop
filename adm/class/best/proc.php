...
$MSG = new Msg();
$tot_num = '1';    //첨부파일 최대갯수
$UPLOAD_DIR = "/home/edufim/www/upfile/best";
...
switch ($type) {
	...
	case 'edit':
		...
		if ($type == 'write') {
			...
			$query = "INSERT INTO ks_best (status, title, exp, price, discountPrice, discountRate, ment01, ment02, upfile01, realfile01, userip, rTime) VALUES";
			$query .= " (0, '$title', '$exp', '$price', '$discountPrice', '$discountRate', '$ment01', '$ment02', '$arr_new_file[1]', '$real_name[1]', '$userip', $rTime)";
			$result = mysql_query($query) or die(mysql_error());
            $msg = '등록되었습니다';

		} else if ($type == 'edit') {
			$query = "UPDATE ks_best SET";
			$query .= " title='$title',";
			$query .= " exp='$exp',";
            $query .= " price='$price',";
	...
	case 'del':
        ...
        $query = "DELETE FROM ks_best WHERE uid='$uid'";
        $result = mysql_query($query) or die(mysql_error());
	...
	case 'able':
        sqlExe("UPDATE ks_best SET status='$status' WHERE uid='$uid'");
        break;
}
