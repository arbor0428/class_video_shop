...
$last_sort_preview = ($num_rows_preview > 0) ? $num_rows_preview + 1 : 0;
?>
...
<div id="content-wrapper" class="d-flex flex-column">
                    ...
                    <div class="card shadow mb-4" style='margin-top:10px;'>
                        <div class="card-body">
                            <form name='frm01' action="./proc.php" method='post'>
                                ...
                                <!-- preview -->
                                <div class="@tbl-st-w01 @tbl-st-w @tbl-st mb20">
                                    <div class="flex-between">
                                        <div class="font-weight-bold text-success mb-2">미리보기</div>
                                        <div class="text-right">
                                            <a href="javascript:void(0)" class="btn btn-sm btn-danger btn-icon-split ml-1" onclick="remove_list_preview();">
                                                <!-- <span class="icon text-white-50">
                                                        <i class="fas fa-check"></i>
                                                    </span> -->
                                                <span class="text">삭제</span>
                                            </a>
                                            <a href="javascript:void(0)" class="btn btn-sm btn-success btn-icon-split ml-1" onclick="add_list_preview();">
                                    ...
                                    <table width="100%" cellpadding='0' cellspacing='0' border='0' width='100%' class='listTable'>
                                        <tr>
                                            <th width='10'><input type="checkbox" name="allChk_preview" value="" onclick="All_chk('allChk_preview','chk_preview[]');"></th>
                                            <th width='100'>비디오</th>
                                            <th width='10'>순서</th>
                                            <th width='1000'>제목</th>
                                            <th width='1000'>설명</th>
                                        ...
                                        <?
                                        while ($row = mysql_fetch_assoc($result_preview)) {
                                            if ($status == '0')     $status = "<span class='ico09'>비활성</span>";
                                            elseif ($status == '1') $status = "<span class='ico03'>활성</span>";
                                        ?>
                                            <tr class='grayLine'>
                                                <td>
                                                    <input type="checkbox" name="chk_preview[]" value="<?= $row['sort'] ?>">
                                                </td>
                                                <td>
                                                    <input type="hidden" name="kollus_video_id_preview[]" id="kollus_video_id_preview_<?= $row['sort'] ?>" value="<?= $row['kollus_video_id'] ?>" data-sort="<?= $row['sort'] ?>" data-value="class_list_preview" onchange="getVideoInfo(this)">
                                                    <a href="javascript:void(0)" onclick="formModal('kollus_video_id_preview_<?= $row['sort'] ?>');">
                                                        <img id="snapshot_preview_<?= $row['sort'] ?>" src="<?= $row['snapshot_url'] ?>" alt="<?= $row['title'] ?>" width="100">
                                                        <span class="icon"><i class="fas fa-search"></i></span>
                                                    </a>
                                                </td>
                                                <td><?= $row['sort'] ?></td>
                                                <td><input type='text' name='title_preview[]' id="title_preview_<?= $row['sort'] ?>" class="form-control" style='width:100%' value='<?= $row['title'] ?>'></td>
                                                <td><input type='text' name='exp_preview[]' class="form-control" style='width:100%' value='<?= $row['exp'] ?>'></td>
                                                <td></td>
                                            </tr>
                                        <? } ?>
                                    </table>
    ...
    <script>
        ...
        last_sort = (parseInt(last_sort)) ? parseInt(last_sort) : 0;

        let last_sort_preview = '<?= $last_sort_preview ?>';
        last_sort_preview = (parseInt(last_sort_preview)) ? parseInt(last_sort_preview) : 0;
        ...
        const formModal = function(value1) {
            ...
            $('#multiFrame').html("<iframe src='/adm/class/kollus_video/index.php?value1=" + value1 + "' name='kollusFrame' style='width:100%;height:680px;' frameborder='0' scrolling='auto'></iframe>");
            $('.multiBox_open').click();
        }
    </script>
