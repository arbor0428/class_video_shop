...
<form name='frm01' method='post' action='<?= $_SERVER['PHP_SELF'] ?>'>
    ...
    <table cellpadding='0' cellspacing='0' border='0' width='100%'>
        <tr style="height:60px;">
            ...
            <td height='30' align='right'>
                <div style="display:flex; align-items: center; justify-content: flex-end;">
                    <select class="board-select" name="field">
                        <option value='title' selected>게시판명</option>
                    </select>
                    <input name="word" type="text" size="20" value='<?= $word ?>' style="margin: 0 4px; display:block; width:200px; height: 34px;"> <a class="btn2 btn blk" href='javascript:document.frm01.submit();'>검색</a>
                </div>
            </td>
        </tr>
        <tr>
            <td colspan='2'>
                <table cellpadding='0' cellspacing='0' border='0' width='100%' class='pTable'>
                    <tr>
                        <th width="5%"><input name='all_chk' type='checkbox' onclick="All_chk('all_chk','chk[]');" style="margin: 0 auto;"></th>
                        <th width="5%">번호</th>
                        <th width="28%">게시판명</th>
                    ...
                    ?>
                            ...
                            <tr>
                                <th style="background-color:#fff;"><input name='chk[]' type='checkbox' value='<?= $uid ?>' style="margin: 0 auto;"></th>
                                <td align='center'><?= $i ?></td>
                                <td align='center'><a onclick="javascript:toclip(<?= $line_num ?>);" style='cursor:hand;'><?= $title ?><br><?= $table_id ?></a></td>
                                <td align='center'><?= $list_mod ?></td>
                                <td align='center'><?= $write_chk ?></td>
                                <td align='center'><?= $read_chk ?></td>
                                <td align='center'><?= $reg_date ?></td>
                                <td align='center'><a href="javascript:reg_modify('<?= $uid ?>');" class="btn btn-success btn-circle btn-sm" title="수정"><i class="fas fa-info-circle"></i></a> <a href="javascript:click_del('<?= $title ?>','<?= $uid ?>')" class="btn btn-danger btn-circle btn-sm" title="삭제"><i class="fas fa-trash"></i></a></td>
                            </tr>
                        <?
                            $i--;
                            $line_num++;
        ...
        <tr>
            <td colspan='2' style='padding:20px 0;' align='right'><a class="btn2 btn blk" href='./index.php?type=write'>등록하기<!-- <img src='/images/common/register.gif'> --></a></td>
        </tr>
    </table>
