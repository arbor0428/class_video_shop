...
function sortData01(s){
	form = document.frm01;

	form.type.value = '';
	form.f_data01.value = s;
	form.uid.value = '';
	form.record_start.value = '';
	form.action = '<?=$PHP_SELF?>';
	form.target = '';
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" style='background:#fafafa;border:1px solid #d1d1d1;padding:5px;margin-bottom:10px;'> 
			...
			<table cellpadding='0' cellspacing='0' border='0' width='100%'>
				...
				?>
					<td align='center'>
						...
						<select name="field" style='height:32px;border:1px solid #dddddd;border-radius:3px;padding:0 5px;float:left;'>
						...
						<input name="word" type="text" style='width:200px;height:32px;border:1px solid #dddddd;border-radius:3px;padding:0 5px;float:left;margin-left:5px;' value='<?=$word?>' onkeypress="if(event.keyCode==13){goSearch();}">
						<a href="javascript:goSearch();" class="big cbtn black" style='float:left;margin-left:5px;'>검색</a>
						</div>
					</td>
				</td>
			</table>
		...
		<td style="border:1px solid #bcbcbc; border-left:0px; border-right:0px; background-color:#f6f6f6; padding:8px 0px; text-align:center;">
			<table cellpadding='0' cellspacing='0' border='0' width='100%'>
				<tr align='center'>
					...
					<td width="13%" class='b-text'>등록일</td>
					<td width="5%" class='b-text'>조회수</td>
				</tr>
			</table>
		</td>
	</tr>
...
if($total_record != '0'){
	...
	while($row = mysql_fetch_array($result)){
		...
		if($userfile01)	$FileIcon = "<img src='/images/ico_save.gif' style='width:auto !important;margin:0 5px;'>";
		else				$FileIcon = '';

		if($date_diff < $newday)	$B_NewIcon = "<img src='/images/mBoardNew.gif' style='width:auto !important;'>";
		else	$B_NewIcon = '';
...
?>
				<tr <?=$bgcolor?> height='35'> 
					...
					<td width="10%" class='b-text' align='center'>[<?=$data01?>]</td>
...
?>
				...
				<tr <?=$bgcolor?> height='35'> 
					...
					<td width="13%" class='b-text-s' align='center'><?=$reg_date?></td>
					<td width="5%" class='b-text-s' align='center'><?=$hit?></td>
				</tr>
