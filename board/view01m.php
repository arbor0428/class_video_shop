		...
		<td>
			<table width="100%" border="0" cellspacing="0" cellpadding="0" class='gTable2'>
				...
				<tr align="" height='30'>
					...
					<th width="52%" class='bbs'>제목</th>
					<th width="28%" class='bbs'>등록일</th>
				</tr>
				...
				<tr height="300" valign="top">
					<td id='imgSize' class='bbs01' colspan='3' style="padding:20 5 10 5"><?=$ment?></td>
				</tr>
...
for($i=1; $i<=$upload_chk; $i++){
	...
	$file_rename = ${'userfile'.$file_num};

	if($file_name){
		$fno++;

		$file_encode = urlencode($file_name);
?>
					...
					<td class='bbs01' colspan='2'>
						<table cellpadding='0' cellspacing='0' border='0' width='100%'>
							...
							<?
								if($download_chk){
							?>
								<td width='80' align='right'><a href="javascript:file_down_m('<?=$file_name?>','<?=$file_rename?>');" class='mini cbtn black'>다운로드</a></td>
							<?
								}
							?>
							</tr>
