...
<?
if($GBL_MTYPE == 'A'){
	if(!($table_id == 'table_1668647496' || $table_id == 'table_1668647506' || $table_id == 'table_1668647516' || $table_id == 'table_1668647523')){
?>
				<div class="tbl-st-row clearfix"> 
					<div class="tbl-st-col col-1">공지</div>
					<div class="tbl-st-col col-2" style="height: 50px; display: flex; align-items:center;"> <input name="notice_chk" type="checkbox" value='1' <?if($notice_chk=='1') echo 'checked';?>> 체크하실 경우 리스트의 최상단에 출력됩니다</div>
				</div>
<?
	}
}else{
...
?>
				...
				<div class="tbl-st-row clearfix"> 
					...
					<div class="tbl-st-col col-2">
						<input name="cade03" type="text" style='width:200px;height:30px;' value='<?=$cade03?>'>
						<select class="ipmt10" onchange="document.FRM.cade03.value=this.options[this.selectedIndex].value;"  style='height:30px; border:1px solid #e1e1e1; border-radius:4px; margin-right:4px;'>
							<option value="">:: 직접입력 ::</option>
							<?
								$sql_c = "select distinct(cade03) from tb_board_list where table_id='$table_id'";
								$result_c = mysql_query($sql_c);
								while($row_c = mysql_fetch_array($result_c)){
									$cade03Txt = $row_c["cade03"];
									if($cade03Txt){
							?>
								<option value="<?=$cade03Txt?>"><?=$cade03Txt?></option>
							<?
									}
								}								
							?>
						</select>
						<!-- <select class="ipmt10" name="cade03sort" style='height:30px; border:1px solid #e1e1e1; border-radius:4px; margin-right:4px;'>
							<option value="" <?if(!$cade03sort)	echo'selected';?>>:: 오름차순 ::</option>
							<option value="1" <?if($cade03sort=='1')	echo'selected';?>>:: 내림차순 ::</option>
						</select> -->
					</div>
				</div>
				<?
				...
				<?
					if($table_id == 'table_1668558028'){
				?>
				
				<div class="tbl-st-row clearfix"> 
					<div class="tbl-st-col col-1">링크주소</div>	
					<div class="tbl-st-col col-2">
						<input name="data01" type="text" style='width:99%;' value="<?=$data01?>" placeholder='http:// 를 포함한 주소를입력해주세요'>
					</div>
				</div>
				<style>
					.mentNone{display:none;}
				</style>
				<?
					}	
				?>
				<?
					if($table_id == 'table_1668642466' || $table_id == 'table_1668642480'){
				?>
				
				<div class="tbl-st-row-wrap clearfix">
					<div class="tbl-st-row clearfix"> 
						<div class="tbl-st-col col-1">실적연도</div>
						<div class="tbl-st-col col-2"><input name="data01" id="user_name" type="text" style='width:98%;' value="<?=$data01?>"></div>
					</div>
					<div class="tbl-st-row clearfix"> 
						<div class="tbl-st-col col-1">고객사</div>
				...
				<div class="tbl-st-row clearfix"> 
					<div class="tbl-st-col col-1">영문</div>
					<div class="tbl-st-col col-2"><input name="data01" id="user_name" type="text" style='width:98%;' value="<?=$data01?>"></div>
				</div>
				<style>
					.mentNone{display:none;}
...
<div class="con clearfix">
	...
	?>
		<tr>
			<td align='right' height='50'>
				<a href="javascript:check_form();" class="btn2 btn grn">등록</a>&nbsp;
				<a href="javascript:reg_list();" class="btn2 btn red"><!--<a href="javascript:reg_list();">-->취소</a>
			</td>
		</tr>
	<?
	}else{
	?>
		<tr>
			<td align='right' height='50'>
				<a href="javascript:check_form();" class="btn2 btn grn">수정</a>&nbsp;
				<a href="javascript:reg_del();" class="btn2 btn red">삭제</a>&nbsp;
				<a href="javascript:reg_list();" class="btn2 btn blk">목록보기</a>
			</td>
		</tr>
	<?
	}
